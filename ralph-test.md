...
- [2] TEST-2-FAIL-ONCE-THEN-SUCCESS: Read this file (ralph-test.md) and check this task's current marker. If it's [ ] or [1], intentionally fail by running `bash -c "exit 1"`. If it's [2], create file `test2-retry-once.txt` with content "succeeded after 1 retry" and mark [x].
